/*
    ...
    minore rispetto ai precedenti, se vengono rispettati alcuni vincoli sull'input dei dati, in tempo θ(n)
        ...
        • Counting Sort - θ(n) - in input dobbiamo avere valori interi, dove n è la lunghezza dell'array e m è pari a max(A)-min(A)+1
        • Radix Sort - θ(n) - ha bisogno di più del doppio della memoria dei metodi in place e di essere implementato attraverso un algortimo stabile
...
#include <cstdlib>
using namespace std;
//Function per ritornare il massimo in un array
template<class T>
...
class Algorithm {
    ...
    public:
        ...
        // Radix Sort
        template <class T> void radixsort(T a[],int n);
        // Bucket Sort
...
// Radix Sort
template<class T>
Algorithm::void radixsort(int arr[], int n){
...
void Algorithm::bubbleSort(typename vector<T>::iterator begin, typename vector<T>::iterator end){
    ...
    for (i = begin; i != end; i++)
        for (j = end; j != i; j--)
        	//Se l'elemtno j-1 è minore dell'elemento j, swap
            compswap(*(j-1),*j);
}
...
void Algorithm::insertionSort(typename vector<T>::iterator begin, typename vector<T>::iterator end){
    ...
    for ( j = begin+1; j != end; j++ ){
        key = *j;
        i = j-1;
        //Se trovi un valore > key, scambia i valori e riparti col ciclo
        while ( i >= begin && *i > key ){
            *(i+1) = *i;
            i--;
        }
